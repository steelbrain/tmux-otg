...
- **`src/http.rs`** — HTTP layer: router, handlers, the SSE relay, a security-headers middleware (`Content-Security-Policy`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`), and the tiny inline HTML/JS UI. Routes: `GET /`, `GET /view/{name}`, `GET /stream/{name}`, `GET /healthz`, `GET /favicon.png`, `GET /apple-touch-icon.png`, plus a 404 fallback. The `/stream` handler subscribes to the `Hub` and relays each `PaneUpdate`; the actual capture loop lives in `hub.rs`. The two icon routes serve PNGs embedded from `assets/` via `include_bytes!` (no asset files to ship), so the UI stays self-contained. Contains `escape_html()` and the HTML/SSE templating.
