...
- **`src/http.rs`** — HTTP layer: router, handlers, the SSE relay, a security-headers middleware (`Content-Security-Policy`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`), and the tiny inline HTML/JS UI. Routes: `GET /`, `GET /view/{name}`, `GET /stream/{name}`, `GET /healthz`, `GET /favicon.svg`, `GET /favicon.png`, `GET /apple-touch-icon.png`, `GET /fonts/jetbrains-mono-light.woff2`, `GET /fonts/jetbrains-mono-bold.woff2`, plus a 404 fallback. The `/stream` handler subscribes to the `Hub` and relays each `PaneUpdate`; the actual capture loop lives in `hub.rs`. The icon and font routes serve assets embedded from `assets/` via `include_bytes!` (no asset files to ship), so the UI stays self-contained; the favicon is an SVG (primary) with a 32x32 PNG fallback, `/apple-touch-icon.png` is the 180x180 iOS home-screen icon, and the two woff2 files are the embedded JetBrains Mono webfont (OFL-1.1, `assets/fonts/OFL.txt`) the pane is rendered in — served same-origin so the CSP stays `font-src 'self'`. Contains `escape_html()` and the HTML/SSE templating.
...
7. **Output escaping.** All session names rendered into HTML go through `escape_html()` (escapes `& < > " '`). Pane text and the stream URL are JSON-encoded (`serde_json`) so embedded newlines/quotes can't break SSE framing or the inline `<script>`. Pane snapshots are captured with colour escapes (`tmux capture-pane -e`) and the browser turns those SGR sequences into styled `<span>` elements — but **only** via `document.createElement` + `textContent` (never `innerHTML`), and the only styles applied are colours computed from numeric SGR codes (a fixed palette / xterm-256 / 24-bit rgb), never from pane text. So coloured output is neither an HTML- nor a CSS-injection vector. Preserve this encoding — and the `textContent`-only, numeric-colours-only rendering — for any new output paths.
8. **Defense-in-depth headers.** Every response carries a restrictive `Content-Security-Policy` (no external resources; inline `<style>`/`<script>` only; images, fonts, and SSE connections confined to our own origin via `img-src 'self'` / `font-src 'self'` / `connect-src 'self'`; `frame-ancestors 'none'`), plus `X-Content-Type-Options: nosniff` and `Referrer-Policy: no-referrer`. The only image served is the embedded same-origin favicon and the only font the embedded same-origin JetBrains Mono webfont, so `img-src` and `font-src` stay `'self'` — never widen them to external hosts or `data:`. If you add external assets you will need to widen the CSP — do so as narrowly as possible.
