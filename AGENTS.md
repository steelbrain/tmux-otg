...
- **`src/http.rs`** — HTTP layer: router, handlers, the SSE relay, a security-headers middleware (`Content-Security-Policy`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`), and the tiny inline HTML/JS UI. Routes: `GET /`, `GET /view/{name}`, `GET /stream/{name}`, `GET /healthz`, `GET /favicon.svg`, `GET /favicon.png`, `GET /apple-touch-icon.png`, plus a 404 fallback. The `/stream` handler subscribes to the `Hub` and relays each `PaneUpdate`; the actual capture loop lives in `hub.rs`. The icon routes serve assets embedded from `assets/` via `include_bytes!` (no asset files to ship), so the UI stays self-contained; the favicon is an SVG (primary) with a 32x32 PNG fallback, and `/apple-touch-icon.png` is the 180x180 iOS home-screen icon. Contains `escape_html()` and the HTML/SSE templating.
...
8. **Defense-in-depth headers.** Every response carries a restrictive `Content-Security-Policy` (no external resources; inline `<style>`/`<script>` only; images and SSE connections confined to our own origin via `img-src 'self'` / `connect-src 'self'`; `frame-ancestors 'none'`), plus `X-Content-Type-Options: nosniff` and `Referrer-Policy: no-referrer`. The only image served is the embedded same-origin favicon, so `img-src` stays `'self'` — never widen it to external hosts or `data:`. If you add external assets you will need to widen the CSP — do so as narrowly as possible.
